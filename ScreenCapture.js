...
function CaptureAndSaveAllPage(outputName) {
    ...
    html2canvas(region, {
        ...
        onrendered: function (canvas) {
            canvas.toBlob(function (blob) {
                saveAs(blob, outputName + ".jpg");
            });
        },
    });
}

function CaptureAndSavePartialPage(outputName, selectorElement) {
    let region = document.getElementById(selectorElement);
    html2canvas(region, {
        useCORS: true,
        allowTaint: true,
        async: false,
...
function CaptureAllPage(targetElement) {
    let region = document.querySelector("body");
    html2canvas(region, {
        useCORS: true,
        allowTaint: true,
        ...
        foreignObjectRendering: true,
        onrendered: function (canvas) {
            let imageSource = canvas.toDataURL('image/jpeg');
...
function CapturePartialPage(selectorElement, targetElement) {
    let region = document.getElementById(selectorElement);
    html2canvas(region, {
        useCORS: true,
        allowTaint: true,
